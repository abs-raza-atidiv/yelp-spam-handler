	...
	if id_list:
		...
		for i in range(latest_email_id,first_email_id-1, -1):		 
			...
			try:
				...
				result = mail.uid('COPY', msg_uid, 'incorrect_vl_processed')
				if result[0] == 'OK':
					_ , data1 = mail.uid('STORE', msg_uid , '+FLAGS', '(\Deleted)')
					mail.expunge()
				print("processed emails moved to incorrect_vl_processed label")	
			except Exception as e:
				print(str(e))
